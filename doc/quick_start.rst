...
To create your package, you need to clone the ``farc-hd-sklearn`` repository:

.. prompt:: bash $

  git clone https://github.com/scikit-learn-contrib/farc-hd-sklearn.git

Before to reinitialize your git repository, you need to make the following
changes. Replace all occurrences of ``farc_hd``, ``farc-hd-sklearn``, or
``farc-hd-sklearn`` with the name of you own project. You can find all the
occurrences using the following command:

.. prompt:: bash $

  git grep farc_hd
  git grep farc-hd-sklearn
  git grep farc-hd-sklearn
